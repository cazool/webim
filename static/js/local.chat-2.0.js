...
// Create a socket
//socket = new WebSocket('ws://' + window.location.host + '/client/join?uname=tsuibin');
...
//    }
    ...
    _im.defaultOptions = {
        ...
        sendHandler: function ($chatMain) {
            ...
            // 获取即将发送的内容
            if (content) {
                var sender = $chatMain.attr("sender"); //发送者id
                var receiverId = $chatMain.attr("id"); //接受者id
                var obj= {
                 "from":sender,
                 "to":receiverId,
                 "text":content
